...
- `versions/v0.4/` - v0.4のアーカイブ（最新ビルド）

## 次の実装候補（Claude へのバトンタッチ）
- エコー／ゲート活用を見越した敵バリエーション（ゲート封鎖やエコー追尾など）
- ステージの横方向スクロール導入とゴール演出（ゲートとの連動を調整）
- リスクとリワードの見直し（エネルギー上限拡張やコンボ減衰の再バランス）
- サウンドレイヤー追加（ゲート、エコー、フェーズ切替に専用SE）
- パーティクル／ポストプロセス強化で次元演出を明瞭化
