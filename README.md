...
- `versions/v0.6/` - v0.6のアーカイブ（最新ビルド）

## 次の実装候補（Claude へのバトンタッチ）
- Echo Swap を前提とした敵/ギミックの対抗策（パルス耐性や追尾行動の調整）
- フェーズごとのステージ進行やゴール演出を追加し、エネルギー管理に長期的な意味付けをする
- サウンドレイヤーとリアクティブなBGMでエコー/パルス/ゲートの手触りを強化
- スコアリングのチューニング（Echo Swap 多用時のコンボ暴走を抑える報酬曲線）
- 新フェーズギミック（時間停止領域やエネルギードレインなど）
