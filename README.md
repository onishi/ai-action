...
- **複数ステージシステム**（6ステージ構成）（v0.9, v0.16で拡張）
...
- **6つのステージ**:
  ...
  6. Astral Void（6000px、280秒）- 宇宙空間（v0.16）
- **タイムアタック**: 各ステージに制限時間を設定
- **ランク評価**: クリアタイムに応じてS/A/B/Cランクを付与
- **タイムボーナス**: 制限時間より早くクリアすると残り時間×10点のボーナス
- **動的レベル生成**: ステージ幅に応じてプラットフォーム、オーブ、パワーアップ、ピット、シャードを自動配置（v0.11で再調整）
- **ステージ進行**: スコアとライフを引き継いで次ステージへ
- **全ステージクリア**: 最終ステージクリアでエンディング画面

### ステージテーマ＆地形ビジュアル（v0.15, v0.16で拡張）
- **Aurora Steppe** (Phase Introduction): 柔らかな段丘と浮遊するオーロラ雲を導入し、SOLID/ETHERで景観と足場の段差が大きく変化
- **Crystal Expanse** (Echo Mastery): 尖塔・輪環のパララックス演出とシャード状の狭い足場で空間把握を要求
- **Volcanic Rift** (Final Dimension): 噴き上がるマグマ背景と崩れそうな柱状足場を配置し、ピットに溶岩の深みを追加
- **Neon Cityscape** (v0.16): ビルのシルエットとネオンサイン、明滅する光の粒子でサイバーパンクの街並みを表現
- **Deep Ocean Trench** (v0.16): 揺れる珊瑚、漂うクラゲ、生物発光粒子で深海の神秘的な雰囲気を演出
- **Astral Void** (v0.16): 星雲、回転する小惑星、瞬く星々で広大な宇宙空間を表現
- 各テーマでプラットフォーム陰影・縁取り・ピット演出を動的に切り替え、フェーズごとの色調と合わせて「ガラッと」見た目が変わるよう調整
...
- `versions/v0.16/` - v0.16のアーカイブ（最新ビルド）

## 次の実装候補（Codex へのバトンタッチ）
- テーマ連動ギミックを追加（Auroraで風に流される気流、Crystalでリズム光壁、Volcanicで周期噴火など）
