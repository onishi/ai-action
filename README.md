...
- **フェーズシャードによる共鳴ブースト**（ゲート廃止後の新収集要素）（v0.11）
...
- フェーズシャードを集めると Apex 共鳴ゲージが蓄積し、3つで自動的に共鳴発動（v0.11）
...
- フェーズシャードで Apex 共鳴ゲージを追加チャージし、SURGE/STORM に頼らず共鳴を発生（v0.11）
...
- フェーズシャード経由での共鳴発動により、移動ギミックに頼らず攻撃チャンスを得られる（v0.11）
...
- フェーズシャードによる共鳴ゲージが最大になると自動的にApex発動（v0.11）
...
### ステージシステム（v0.9）
- **3つのステージ**: Phase Introduction（3200px）、Echo Mastery（4000px）、Final Dimension（5000px）
- **タイムアタック**: 各ステージに制限時間を設定（180秒、210秒、240秒）
- **ランク評価**: クリアタイムに応じてS/A/B/Cランクを付与
- **タイムボーナス**: 制限時間より早くクリアすると残り時間×10点のボーナス
- **動的レベル生成**: ステージ幅に応じてプラットフォーム、オーブ、パワーアップ、ゲート、ピットを自動配置（v0.11で再調整）
...
- `versions/v0.11/` - v0.11のアーカイブ（最新ビルド）

## 次の実装候補（Codex へのバトンタッチ）
- ピットを活かした新ギミック（動く穴、フェーズごとに出現する穴など）の追加
- ステージ固有ギミック（浮遊足場や崩落床など）を追加し、Apex活用の選択肢を広げる
- ステージ終盤にミニボスを配置し、Apex有無で攻略ルートが変化する戦闘を設計
- タイムアタック向けにハイスコア＆ゴースト記録を実装し、リプレイ性を強化
- サウンド/BGMと演出（パワーアップ・Apex発動時の専用SE、視覚エフェクトの調整）
- 難易度モードやステージ分岐を検討し、クリア後の周回リプレイをサポート
