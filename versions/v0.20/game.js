...
const stages = [
    {
        ...
        theme: {
            key: 'aurora-steppe',
            label: 'Aurora Steppe',
            summary: '柔らかな段丘と極光がフェーズ感覚を整える入門区画',
            hint: 'SOLIDで段丘を駆け上がり、ETHERで浮遊足場を渡ろう',
    ...
    },
    {
        ...
        theme: {
            ...
            summary: '音律に合わせて結晶が輝く空域で、シャード導線を読み解け',
            hint: 'リズムよくフェーズを切り替え、狭い結晶足場を連続ジャンプ',
    ...
    },
    {
        ...
        theme: {
            key: 'volcanic-rift',
            label: 'Volcanic Rift',
            summary: '噴き上がるマグマと崩落柱が待つ終局領域、共鳴で駆け抜けろ',
            hint: 'SURGEでスピードを上げ、噴火のタイミングでEcho Swapを決めろ',
    ...
    },
    {
        ...
        theme: {
            ...
            summary: 'ネオンが脈打つ多層ビル群で速度と精度を競うメトロプレックス',
            hint: 'SOLIDでビルの稜線を滑走し、ETHERで光の足場とホログラムを拾い続けよう',
    ...
    },
    {
        ...
        theme: {
            ...
            summary: '潮流が揺らす浮遊珊瑚と生物発光が導く深海アスレチック',
            hint: 'SOLIDで安定した岩盤を拾い、ETHERで潮流に乗る浮遊足場へリレーしよう',
    ...
    },
    {
        ...
        theme: {
            key: 'astral-void',
            label: 'Astral Void',
            summary: '星雲と小惑星が漂う無重力空間でフェーズ管理が命綱となる終章',
            hint: 'SOLIDで重力安定プラットフォームを確保し、ETHERで浮遊リングとワープ軌道を繋げ',
...
function initializeStageAmbience(stage) {
    ...
    if (stage.theme.key === 'aurora-steppe') {
        ...
        // ネオン都市 - 軽量化版
        const buildingCount = Math.min(4, Math.max(3, Math.floor(stage.width / 1200)));
        ...
        // ネオンサイン
        const signCount = Math.min(5, Math.max(4, Math.floor(stage.width / 900)));
        ...
        // 深海 - 軽量化版
        const coralCount = Math.min(4, Math.max(3, Math.floor(stage.width / 1400)));
        ...
        // クラゲ
        const jellyfishCount = Math.min(3, Math.max(2, Math.floor(stage.width / 1800)));
        ...
        // 宇宙 - 軽量化版
        const nebulaCount = Math.min(3, Math.max(2, Math.floor(stage.width / 2000)));
        ...
        // 小惑星
        const asteroidCount = Math.min(4, Math.max(3, Math.floor(stage.width / 1500)));
...
let frameCounter = 0;
let gameState = 'stageintro'; // stageintro, playing, gameover, stageclear, allclear
let score = 0;
let combo = 0;
let comboTimer = 0;
let lives = 3;
...
let apexTimer = 0;
let apexCharge = 0;
let stageClearRewardGranted = false;
...
window.addEventListener('keydown', (e) => {
    ...
    if (gameState === 'allclear' || gameState === 'gameover') {
        if (key === ' ') {
            resetGame();
            keys[' '] = false;
            return;
    ...
    if (!keys[key]) {
        ...
        if (key === 'Shift' && !shiftHeld) {
            ...
            shiftHeld = true;
        } else if ((lowerKey === 'q') && !echoSwapRequested) {
...
function updatePlayer() {
    ...
    if (player.x >= GOAL_X && gameState === 'playing') {
        ...
        if (currentStageIndex < stages.length - 1) {
            ...
            const finalBonus = Math.max(0, (stage.timeLimit - stageClearTime) * 10);
            if (finalBonus > 0) {
                score += finalBonus;
            }
...
function proceedToNextStage() {
    ...
    if (!stageClearRewardGranted) {
        const stage = stages[currentStageIndex];
        const timeBonus = Math.max(0, (stage.timeLimit - stageClearTime) * 10);
        if (timeBonus > 0) {
            score += timeBonus;
        }
        stageClearRewardGranted = true;
...
function drawStageBackdrop() {
    ...
    for (const layer of stageBackdrop.layers) {
        if (layer.type === 'cloud') {
            ...
            // 触手を2本に削減（パフォーマンス最適化）
            for (let i = 0; i < 2; i++) {
                const tentacleX = x - layer.size * 0.25 + (i * layer.size * 0.5);
                ctx.strokeStyle = layer.glow;
                ctx.lineWidth = 2;
                ctx.beginPath();
...
function drawUI() {
    ...
    ctx.font = '14px "Arial"';
    const stage = stages[currentStageIndex];
    let elapsedTime = stageClearTime;
    if (gameState === 'playing') {
        elapsedTime = Math.floor((Date.now() - stageStartTime) / 1000);
...
function drawStageIntroScreen() {
    const stage = stages[currentStageIndex];
    const theme = stage.theme ?? {};
    const accent = currentStageTheme?.palettes?.SOLID?.accent ?? '#ffe066';

    ctx.save();
    const overlay = ctx.createLinearGradient(0, 0, 0, canvas.height);
    overlay.addColorStop(0, 'rgba(6, 12, 32, 0.85)');
    overlay.addColorStop(1, 'rgba(4, 8, 20, 0.75)');
    ...
    ctx.fillText('STAGE READY', canvas.width / 2, 140);

    ctx.font = 'bold 30px "Arial"';
    ctx.fillStyle = accent;
    ctx.fillText(`Stage ${stage.id}: ${stage.name}`, canvas.width / 2, 190);

    if (theme.label) {
        ctx.font = '20px "Arial"';
        ctx.fillStyle = 'rgba(255,255,255,0.85)';
        ctx.fillText(`[${theme.label}]`, canvas.width / 2, 226);
    }
    ...
    if (theme.summary) {
        ...
        for (const char of theme.summary) {
            buffer += char;
            if (buffer.length >= maxCharsPerLine) {
                summaryLines.push(buffer);
                buffer = '';
            }
        }
        if (buffer.length > 0) {
        ...
        summaryLines.forEach((line, index) => {
            ctx.fillText(line, canvas.width / 2, baseY + index * 24);
        });
    }

    const instructions = [
        { key: 'SPACE', desc: 'ステージ開始' },
        { key: 'SHIFT', desc: '相互次元シフト' },
        { key: 'Q', desc: 'Echo Swap' },
        { key: 'R', desc: 'ステージ再スタート' }
    ];
    ...
    instructions.forEach((item, index) => {
        ...
        const boxHeight = 38;
        const gap = 12;
        const totalWidth = instructions.length * boxWidth + (instructions.length - 1) * gap;
        ...
        ctx.fillText(item.key, x + boxWidth / 2, y + 20);
        ctx.font = '13px "Arial"';
        ctx.fillStyle = 'rgba(255,255,255,0.8)';
        ctx.fillText(item.desc, x + boxWidth / 2, y + 34);
    });

    const hintText = theme.hint || 'ヒント: Shift と Echo Swap を組み合わせて導線を作ろう';
    ctx.font = '14px "Arial"';
    ctx.fillStyle = 'rgba(255,255,255,0.7)';
    ctx.fillText(`ヒント: ${hintText}`, canvas.width / 2, canvas.height - 80);
...
function drawGameOver() {
    ...
    ctx.font = '18px "Arial"';
    ctx.fillText('SPACE / R でリスタート', canvas.width / 2, canvas.height / 2 + 60);
...
function drawStageClearScreen() {
    ...
    ctx.fillStyle = '#fff';
    ctx.font = '18px "Arial"';
    ctx.fillText('SPACE: 次のステージへ進む', canvas.width / 2, 490);
    ctx.fillText('N: NEXT / R: STAGE RESTART', canvas.width / 2, 520);

    ctx.font = '14px "Arial"';
    ctx.fillStyle = 'rgba(255,255,255,0.8)';
    ctx.fillText('ボーナス: 残りタイム × 10 は進行時に自動加算されます', canvas.width / 2, 552);
...
function drawAllClearScreen() {
    ...
    ctx.fillStyle = '#fff';
    ctx.font = '18px "Arial"';
    ctx.fillText('SPACE: 新しいランを開始', canvas.width / 2, 460);
    ctx.fillText('R: Play Again', canvas.width / 2, 490);

    ctx.font = '14px "Arial"';
    ctx.fillStyle = 'rgba(255,255,255,0.78)';
    ctx.fillText('タイムボーナスは最終スコアに反映済みです', canvas.width / 2, 520);
