...
let frameCounter = 0;
let gameState = 'playing'; // playing, gameover, goal
let score = 0;
let combo = 0;
let comboTimer = 0;
let lives = 3;
...
const phaseGates = [
    ...
    { id: 'SOLID_GATE_MID1', phase: 'SOLID', x: 1450, y: 420, width: 42, height: 70, link: 'SOLID_GATE_MID2', cooldown: 0 },
    { id: 'SOLID_GATE_MID2', phase: 'SOLID', x: 2150, y: 420, width: 42, height: 70, link: 'SOLID_GATE_MID1', cooldown: 0 },
    { id: 'ETHER_GATE_MID1', phase: 'ETHER', x: 1300, y: 370, width: 48, height: 72, link: 'ETHER_GATE_MID2', cooldown: 0 },
    { id: 'ETHER_GATE_MID2', phase: 'ETHER', x: 2450, y: 270, width: 48, height: 72, link: 'ETHER_GATE_MID1', cooldown: 0 }
];
...
function handleEchoSwap() {
    ...
    if (echoSwapCooldown > 0 || phaseEchoes.length === 0 || phaseEnergy < ECHO_SWAP_COST) {
        return;
    }
    ...
    phaseShockwaves.push({
        ...
        radius: 20,
        growth: 6,
        maxRadius: ECHO_SWAP_PULSE_RADIUS,
        alpha: 0.45,
        phase: originPhase,
...
function drawUI() {
    ...
    // スコア・コンボ・ライフ表示
    ctx.fillStyle = 'rgba(0, 0, 0, 0.4)';
    ctx.fillRect(canvas.width - 180, 20, 160, 90);
    ...
    // ライフ表示
    ctx.fillStyle = '#fff';
    ctx.fillText('Lives:', canvas.width - 165, 98);
    for (let i = 0; i < lives; i++) {
        ctx.fillStyle = '#ff6b6b';
        ctx.fillRect(canvas.width - 110 + i * 20, 88, 15, 15);
    }
...
function drawGoalScreen() {
    ...
    ctx.font = 'bold 48px "Arial"';
    ctx.textAlign = 'center';
    ctx.fillText('GOAL!', canvas.width / 2, canvas.height / 2 - 60);

    ctx.font = 'bold 36px "Arial"';
    ctx.fillText(`Final Score: ${score}`, canvas.width / 2, canvas.height / 2);

    ctx.font = '24px "Arial"';
    ctx.fillText(`Max Combo: x${combo}`, canvas.width / 2, canvas.height / 2 + 40);
